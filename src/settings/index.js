...
export async function loadSettings(rootDir) {
    ...
    try {
        ...
        if (!result.success) {
            console.warn("[mdsvr] settings.json validation errors:", result.error.flatten());
...
export function watchSettings(rootDir, onChange) {
...
export async function validateSettingsFile(rootDir) {
    const settingsPath = path.join(rootDir, "settings.json");
    try {
        const raw = await fs.readFile(settingsPath, "utf-8");
        const parsed = JSON.parse(raw);
